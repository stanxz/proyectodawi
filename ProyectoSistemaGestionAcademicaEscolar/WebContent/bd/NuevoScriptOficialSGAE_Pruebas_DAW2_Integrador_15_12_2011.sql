...
CREATE  TABLE `certificadoas` (
  ...
  `CONTENIDODOCCERT` BLOB NULL DEFAULT NULL ,
  `DOCENVIADO` INT(11) NULL DEFAULT NULL ,
  `motivo_CODMOTIVO` INT(11) NOT NULL ,
  PRIMARY KEY (`CODCERTIFICADO`) ,
  INDEX `CERTIFICADOAS_CITA_FK` (`CODCITA` ASC) ,
  INDEX `fk_certificadoas_motivo1` (`motivo_CODMOTIVO` ASC) ,
  CONSTRAINT `CERTIFICADOAS_CITA_FK`
    FOREIGN KEY (`CODCITA` )
    REFERENCES `cita` (`CODCITA` ),
  CONSTRAINT `fk_certificadoas_motivo1`
    FOREIGN KEY (`motivo_CODMOTIVO` )
    REFERENCES `motivo` (`CODMOTIVO` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
...
-- Dump completed on 2011-12-15  1:07:08
